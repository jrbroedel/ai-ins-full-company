...
VNet, both subnets, the private DNS zone, the Postgres server, the storage account, and the
Key Vault. Read its own header — it says so. It does **not** contain the VM, its NIC, its
public IP, its NSG, its managed identity, or the Key Vault role assignment. Since 2026-08-16
it does define an **app subnet** — somewhere for a VM to attach, which is not a VM. None of
the VM layer itself has ever been expressed as IaC in any form — no Bicep, no Terraform, no
ARM template, anywhere
in this repo.

**The template cannot help you here, and this was verified rather than assumed:**

1. **Its generated names do not match the deployed ones, and no parameter fixes that.** Every
   name derives from one `uniqueString(resourceGroup().id)` call, so the storage account and
   Key Vault suffixes must be identical — the real ones are `luxautosa**91a2e1**` and
   `luxauto-kv-**90a311**`. The live names were generated by hand before the template was
   written to describe them.
2. **It is a parallel-environment template, not a description of production.** As of
   2026-08-16 it deploys a *fresh* data layer into a *new* resource group, with `baseName`
   required so it cannot be run against `luxauto-rg` by habit. Deploying it is not a step in
   a VM rebuild and never was. See ADR 0008's addendum.

Both points concern the **data** layer, which a VM rebuild does not recreate anyway (see
[Scope](#scope-what-a-vm-rebuild-does-and-does-not-touch)). For the VM layer there is simply
nothing to run: recover the real definitions with `az`, below.
...
3. **Odoo binds `0.0.0.0:8069`/`:8072`, not loopback.** ADR 0009 describes the nginx proxy
   *target* as `127.0.0.1:8069`, which is accurate but easy to misread as the bind address.
   ...
   control (step 4).
4. **`infra/bicep/main.bicep` cannot reproduce the deployed resources — still true, but it is
   no longer *armed*.** Re-derived live and confirmed 2026-08-16: its `uniqueString()`-derived
   names give the storage account and Key Vault the same suffix while the real ones are
   `91a2e1` and `90a311`; a second mismatch was found (it interpolated the raw `location`, so
   it produced `luxauto-eastus2...` where the live private DNS zone is `luxauto-eus2...`,
   confirmed by `dig`); and it omitted `luxauto-app-subnet` (`10.20.3.0/24`), where the VM
   actually is. The dangerous part was that its own header told you to deploy it into
   `luxauto-rg` with defaults, which would have **succeeded** and silently doubled the
   infrastructure. **It has been repurposed** into a parallel-environment template: `baseName`
   is now required with no default, so that command fails instead of running; the region
   abbreviation and app subnet are fixed. It still **cannot** describe or adopt production, and
   it has **never been deployed** — see ADR 0008's addendum. This does not affect step 2's
   instruction to recover real values with `az`.
