...
# Addendum: the re-provisioning gap is documented, and still unverified (2026-08-15)

**Status:** the open item is closed as *documented*; it is explicitly NOT closed as *tested*.
**Amends:** the Consequences line above about host state outside version control, and the project's oldest open backlog item ("no documented re-provisioning path exists for `luxauto-odoo`").

[`docs/runbooks/vm-rebuild.md`](../runbooks/vm-rebuild.md) now exists: an ordered, dependency-first rebuild procedure for this host, written from a live enumeration of the running VM rather than from these ADRs. It tags every step by where its source data actually lives - version-controlled, recoverable via `az` against the existing subscription, or host state that exists nowhere else - and reproduces in full the two files this ADR's addendum deliberately keeps out of the repository (`/usr/local/sbin/luxauto-odoo-deploy-ctl` and `/etc/sudoers.d/10-ghrunner-deploy`), since for those the runbook is the only recovery path.

**The caveat is carried forward rather than retired, and it is the important half of this note.** The runbook has never been executed. No rebuild and no parallel test VM has ever been stood up from it, and it names eleven steps that have never been performed even once - creating the VM (there is no IaC for it), issuing a new managed identity and re-granting Key Vault, reconstructing the NSG, `odoo.conf` and the nginx config, and installing the runner from scratch among them. Section 5's reboot test is not evidence for any of that: it demonstrates that an **already-registered** runner returns after systemd restarts it, which is a narrower claim than "the runner can be freshly installed by following only this runbook." The document says so about itself in its own confidence section; this note repeats it here so the distinction survives being read from the ADR side.

Two findings from that investigation bear directly on this ADR and were not previously recorded anywhere:

- **`ghrunner`'s sudo grants and the wrapper match this addendum exactly** - re-verified live via `sudo -l -U ghrunner`, with no wildcard present and `ghrunner` confirmed absent from the `odoo` group. No drift.
- **A fourth non-stock unit pair exists on this host that this ADR never mentions**: `luxauto-expire-policies.service`/`.timer` (ADR 0019). Unlike the runner and the wrapper, those unit files *are* in version control under `infra/systemd/`, and the installed copies are byte-identical - but installing them is still a manual step no deploy performs, so a rebuilt host silently runs without the hourly term-end job until someone remembers. The runbook covers it; this ADR's "host state a `git clone` does not carry" list should be read as including it.

**The natural follow-up, named and deliberately not attempted:** stand up a parallel VM and execute the runbook end to end against it, with its own identity, its own DNS name and a copy of the database rather than production. That is what would convert the runbook from a careful reconstruction into a verified procedure, and it is a larger undertaking than writing it was - not least because it requires a second runner registration, which section 2 warns against leaving in place.
